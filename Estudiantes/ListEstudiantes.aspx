<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="ListEstudiantes.aspx.cs" Inherits="Proyecto_FInal_Escuela_Musica.Estudiantes.ListEstudiantes" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="header-actions">
                    <a href="AddEstudiante.aspx" class="btn btn-success btn-icon">
            ...
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="input-group">
                                <asp:TextBox ID="txtBuscar" runat="server" CssClass="form-control" placeholder="Buscar por cédula, nombre o apellido..." />
                ...
                <div class="table-container">
                    <asp:GridView ID="gvEstudiantes" runat="server" AutoGenerateColumns="False" 
                        ...
                        OnPageIndexChanging="gvEstudiantes_PageIndexChanging" 
                        DataKeyNames="id_estu" PagerStyle-CssClass="pagination" 
                        HeaderStyle-CssClass="thead-dark" ShowHeaderWhenEmpty="true">
                        <Columns>
                            <asp:BoundField DataField="cedula_estu" HeaderText="Cédula" />
                            <asp:BoundField DataField="nombre_estu" HeaderText="Nombre" />
                            <asp:BoundField DataField="apellido_estu" HeaderText="Apellido" />
                            <asp:BoundField DataField="fecha_nacimiento_estu" HeaderText="Fecha Nacimiento" DataFormatString="{0:dd/MM/yyyy}" />
                            <asp:BoundField DataField="telefono_estu" HeaderText="Teléfono" />
                            <asp:BoundField DataField="email_estu" HeaderText="Email" />
                            <asp:TemplateField HeaderText="Acciones" HeaderStyle-CssClass="actions-column" ItemStyle-CssClass="actions-column">
                                <ItemTemplate>
                                    <div class="action-buttons">
                                        <asp:HyperLink runat="server" NavigateUrl='<%# "EditEstudiante.aspx?id=" + Eval("id_estu") %>' 
                                            CssClass="btn btn-sm btn-warning" ToolTip="Editar">
                                            <i class="fas fa-edit"></i>
                                        </asp:HyperLink>
                                        <!-- Cambio importante aquí: 
                                            Reemplazamos el LinkButton por un botón normal con evento onclick personalizado
                                            y agregamos data-id con el ID del estudiante -->
                                        <button type="button" class="btn btn-sm btn-danger" 
                                            title="Eliminar"
                                            onclick="confirmarEliminacionEstudiante(<%# Eval("id_estu") %>, '<%# Eval("nombre_estu") %> <%# Eval("apellido_estu") %>')">
                        ...
                        <EmptyDataTemplate>
                            <div class="empty-state">
                                ...
                                <a href="AddEstudiante.aspx" class="btn btn-primary mt-3">
                                    <i class="fas fa-plus"></i> Agregar Estudiante
                                </a>
                            </div>
                        </EmptyDataTemplate>
    ...
    <!-- Formulario oculto para eliminar estudiantes -->
    <asp:Button ID="btnEliminarEstudiante" runat="server" OnClick="btnEliminarEstudiante_Click" style="display: none;" />
    <asp:HiddenField ID="hdnEstudianteAEliminar" runat="server" />
    ...
    <script>
        ...
        function confirmarEliminacionEstudiante(idEstudiante, nombreEstudiante) {
            iziToast.question({
                timeout: false,
                close: false,
                overlay: true,
                displayMode: 'once',
                title: '¿Estás seguro?',
                message: `Esta acción eliminará permanentemente al estudiante:<br><strong>${nombreEstudiante}</strong>`,
                position: 'center',
                buttons: [
                    ['<button><b>Sí, eliminar</b></button>', function (instance, toast) {
                        instance.hide({}, toast);
                        
                        // Guardar el ID del estudiante a eliminar en el campo oculto
                        document.getElementById('<%= hdnEstudianteAEliminar.ClientID %>').value = idEstudiante;
                        
                        // Disparar el evento de eliminación
                        document.getElementById('<%= btnEliminarEstudiante.ClientID %>').click();
                    }, true],
