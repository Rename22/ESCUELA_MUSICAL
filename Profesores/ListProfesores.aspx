<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="ListProfesores.aspx.cs" Inherits="Proyecto_FInal_Escuela_Musica.Profesores.ListProfesores" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="header-actions">
                    <a href="AddProfesor.aspx" class="btn btn-success btn-icon">
            ...
            <div class="card-body">
                ...
                <div class="table-container">
                    <asp:GridView ID="gvProfesores" runat="server" AutoGenerateColumns="False" 
                        ...
                        DataKeyNames="id_prof" PagerStyle-CssClass="pagination" 
                        HeaderStyle-CssClass="thead-dark" ShowHeaderWhenEmpty="true">
                        <Columns>
                            <asp:BoundField DataField="cedula_prof" HeaderText="Cédula" />
                            <asp:BoundField DataField="nombre_prof" HeaderText="Nombre" />
                            <asp:BoundField DataField="apellido_prof" HeaderText="Apellido" />
                            <asp:BoundField DataField="nombre_espec" HeaderText="Especialidad" />
                            <asp:BoundField DataField="telefono_prof" HeaderText="Teléfono" />
                            <asp:BoundField DataField="email_prof" HeaderText="Email" />
                            <asp:TemplateField HeaderText="Acciones" HeaderStyle-CssClass="actions-column" ItemStyle-CssClass="actions-column">
                                <ItemTemplate>
                                    <div class="action-buttons">
                                        <asp:HyperLink runat="server" NavigateUrl='<%# "EditProfesor.aspx?id=" + Eval("id_prof") %>' 
                                            CssClass="btn btn-sm btn-warning" ToolTip="Editar">
                                            <i class="fas fa-edit"></i>
                                        </asp:HyperLink>
                                        <button type="button" class="btn btn-sm btn-danger" 
                                            title="Eliminar"
                                            onclick="confirmarEliminacionProfesor(<%# Eval("id_prof") %>, '<%# Eval("nombre_prof") %> <%# Eval("apellido_prof") %>')">
                        ...
                        <EmptyDataTemplate>
                            <div class="empty-state">
                                ...
                                <p class="mb-0">No hay profesores registrados en el sistema</p>
                                <a href="AddProfesor.aspx" class="btn btn-primary mt-3">
                                    <i class="fas fa-plus"></i> Agregar Profesor
                                </a>
                            </div>
                        </EmptyDataTemplate>
    ...
    <!-- Formulario oculto para eliminar profesores -->
    <asp:Button ID="btnEliminarProfesor" runat="server" OnClick="btnEliminarProfesor_Click" style="display: none;" />
    <asp:HiddenField ID="hdnProfesorAEliminar" runat="server" />
    ...
    <script>
        ...
        function confirmarEliminacionProfesor(idProfesor, nombreProfesor) {
            iziToast.question({
                timeout: false,
                close: false,
                overlay: true,
                displayMode: 'once',
                title: '¿Estás seguro?',
                message: `Esta acción eliminará permanentemente al profesor:<br><strong>${nombreProfesor}</strong>`,
                position: 'center',
                buttons: [
                    ['<button><b>Sí, eliminar</b></button>', function (instance, toast) {
                        instance.hide({}, toast);
                        
                        // Guardar el ID del profesor a eliminar en el campo oculto
                        document.getElementById('<%= hdnProfesorAEliminar.ClientID %>').value = idProfesor;
                        
                        // Disparar el evento de eliminación
                        document.getElementById('<%= btnEliminarProfesor.ClientID %>').click();
                    }, true],
