<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="ListEvaluaciones.aspx.cs" Inherits="Proyecto_FInal_Escuela_Musica.Evaluaciones.ListEvaluaciones" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="header-actions">
                    <a href="AddEvaluacion.aspx" class="btn btn-success btn-icon">
            ...
            <div class="card-body">
                ...
                <div class="table-container">
                    <asp:GridView ID="gvEvaluaciones" runat="server" AutoGenerateColumns="False" 
                        ...
                        OnPageIndexChanging="gvEvaluaciones_PageIndexChanging" 
                        DataKeyNames="id_eva" PagerStyle-CssClass="pagination" 
                        HeaderStyle-CssClass="thead-dark" ShowHeaderWhenEmpty="true">
                        <Columns>
                            <asp:BoundField DataField="id_eva" HeaderText="ID" Visible="true" />
                            ...
                            <asp:BoundField DataField="tipo_eva" HeaderText="Tipo" />
                            <asp:BoundField DataField="fecha_eva" HeaderText="Fecha" DataFormatString="{0:dd/MM/yyyy HH:mm}" />
                            <asp:BoundField DataField="puntuacion_eva" HeaderText="Puntuación" />
                            <asp:TemplateField HeaderText="Acciones" HeaderStyle-CssClass="actions-column" ItemStyle-CssClass="actions-column">
                                <ItemTemplate>
                                    <div class="action-buttons">
                                        <asp:HyperLink runat="server" NavigateUrl='<%# "EditEvaluacion.aspx?id=" + Eval("id_eva") %>' 
                                            CssClass="btn btn-sm btn-warning" ToolTip="Editar">
                                            <i class="fas fa-edit"></i>
                                        </asp:HyperLink>
                                        <button type="button" class="btn btn-sm btn-danger" 
                                            title="Eliminar"
                                            onclick="confirmarEliminacionEvaluacion(<%# Eval("id_eva") %>, '<%# Eval("nombre_estu") %>')">
                        ...
                        <EmptyDataTemplate>
                            <div class="empty-state">
                                ...
                                <p class="mb-0">No hay evaluaciones registradas en el sistema</p>
                                <a href="AddEvaluacion.aspx" class="btn btn-primary mt-3">
                                    <i class="fas fa-plus"></i> Nueva Evaluación
                                </a>
                            </div>
                        </EmptyDataTemplate>
    ...
    <!-- Formulario oculto para eliminar evaluaciones -->
    <asp:Button ID="btnEliminarEvaluacion" runat="server" OnClick="btnEliminarEvaluacion_Click" style="display: none;" />
    <asp:HiddenField ID="hdnEvaluacionAEliminar" runat="server" />
    ...
    <script>
        ...
        function confirmarEliminacionEvaluacion(idEvaluacion, nombreEstudiante) {
            iziToast.question({
                timeout: false,
                close: false,
                overlay: true,
                displayMode: 'once',
                title: '¿Estás seguro?',
                message: `Esta acción eliminará permanentemente la evaluación de:<br><strong>${nombreEstudiante}</strong>`,
                position: 'center',
                buttons: [
                    ['<button><b>Sí, eliminar</b></button>', function (instance, toast) {
                        instance.hide({}, toast);
                        
                        // Guardar el ID de la evaluación a eliminar en el campo oculto
                        document.getElementById('<%= hdnEvaluacionAEliminar.ClientID %>').value = idEvaluacion;
                        
                        // Disparar el evento de eliminación
                        document.getElementById('<%= btnEliminarEvaluacion.ClientID %>').click();
                    }, true],
